...
export class MultiSelectComponent implements ControlValueAccessor {
   ...
   */
  readonly dataContextMap$: Observable<Record<string, Record<string, unknown>>> = this._options.pipe(
    map(options => {
   ...
   */
  readonly selectionValueMap$: Observable<Record<string, boolean>> = this._internalValue.pipe(
    map(values => {
